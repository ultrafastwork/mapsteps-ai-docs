# WP Video Popup PRO - AI Agent Rules & Workflow

## Project Context

We are working on the "WP Video Popup PRO" WordPress plugin located in `wp-content/plugins/wp-video-popup-pro`.

## Directory Structure

The `ai-docs/wp-video-popup-pro/{developer}/` directory contains documentation to facilitate context handoffs between AI agents.
...
- **Read Context**: Always check `ai-docs/wp-video-popup-pro/{developer}/progress-handoffs/PROGRESS_HANDOFF.md` to understand the current status, recent changes, and immediate next steps.
...
- **1. Update Handoff File**:
  - Update `ai-docs/wp-video-popup-pro/{developer}/progress-handoffs/PROGRESS_HANDOFF.md`.
  - **Status**: Mark as `Completed` (for this session).
  - **Accomplishments**: List what YOU did.
  - **Next Steps**: List what the NEXT agent should do.
- **2. Archive Handoff**:
  - Read the `Current Session` version from the file (e.g., `v2.10+1`).
...
- **3. Renew Handoff**:
  ...
  - **Current Session**: Increment the version (e.g., `v2.10+1` -> `v2.10+2`).
...
- **7. Show the copy-able prompt for the next agent** with following wording: "Please execute the instructions in: ai-docs/wp-video-popup-pro/{developer}/prompts/AGENT_PROMPT.md".

### 4. Commit Changes to Git

- **Commit Changes in ai-docs**: If there are changes in ai-docs, commit them to git.
- **Commit Changes in wp-video-popup-pro**: If there are changes in wp-content/plugins/wp-video-popup-pro, commit them to git.
...
- **Code Style**: Follow the existing coding standards of the project (WordPress Coding Standards).
- **File Structure**: Maintain the existing directory structure.
  - `inc/`: PHP logic and classes.
  - `assets/`: CSS, JS, and image assets.
  - `templates/`: Shortcode or UI templates.
- **Building**:
  - This project uses SASS for styles (`assets/sass/`).
  - Check for existing build scripts if adding new assets.
- **Security**: Always sanitize inputs and escape outputs using WordPress functions.
